...
function [guessWord] = guesserv2(i,listOfWords)
    ...
    %check the word and detect any flags such as the use of spaces,
    %special characters or the guess word not being in the 14855 word
    %dictionary being used for the game. If the requirements are fulfilled
    %function ends and returns the guessword as an output.

    while length(char(guessWord)) ~= 5 || checkerv2(guessWord,listOfWords) == true
